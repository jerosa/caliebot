...
class CaliebotConsole extends Console {
    ...
    write(data, type = "log") {
        ...
        data = CaliebotConsole.parseData(data);
        const { time, message } = CaliebotConsole.COLORS[type];
        const timestamp = time(`[${this.timestamp}]`);
        super[CaliebotConsole.TYPES[type] || "log"](data.split("\n").map(str => `${timestamp} ${message(str)}`).join("\n"));
    }
    ...
    static parseData(data) {
        if (Array.isArray(data)) return data.map(CaliebotConsole.parseData).join("\n");
...
module.exports = CaliebotConsole;
