...
class Caliebot extends Client {
    ...
    permLevelUser(user, guild) {
        ...
        while (permOrder.length && permlvl === 0) {
            const currentLevel = permOrder.shift();
            if (currentLevel.checkId(guild, user)) {
                permlvl = currentLevel.level;
                break;
            }
        }
        ...
        setTimeout(async () => {
            ...
            streamers.forEach(async streamer => {
                ...
                if (stream === undefined && streamer.live) {
                    ...
                    streamer.live = true;
                    await this.setTwitch(streamer.id, streamer);
                    const game = await this.methods.twitch.getGameData(stream.game_id);
    ...
    async checkSubscriber() {
        setTimeout(async () => {
            this.guilds.forEach(async guild => {
                ...
                const subRole = await guild.roles.find("name", settings.subRole);
                if (!subRole) {
                    this.console.error(`No existe el rol de suscriptor ${settings.subRole}`);
                    return;
                }

                const notSubs = guild.members.filter(member => !member.roles.has(subRole.id));
                const reason = "Fin de la suscripcion.";

                notSubs.forEach(async user => {
                    // si no es mod/admin ni un bot, kick
                    if (this.permLevelUser(user, guild) < 2 && !user.user.bot) {
                        if (user.kickable) {
            ...
            // Comprueba cada hora
        }, process.env.CHECK_SUB_INTERVAL * 1000 * 60);
    }
