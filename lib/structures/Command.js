...
class Command {
	...
	constructor(client, file, options = {}) {
		...
		this.usage = options.usage || "Sin manual de uso.";
		this.extended = options.extended || "Sin información extendida.";
	...
	async paginate(message, list, makeEmbed) {
		const msg = await message.channel.send("`Cargando, por favor espere ...`");
	...
	async verifyUser(message, user, options = {}) {
		...
		if (/(#[0-9]{4})$/.test(user)) member = message.guild.members.find(memb => memb.user.tag === user);
		else member = message.guild.members.find(memb => memb.user.username === user);
